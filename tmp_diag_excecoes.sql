with alvo(cpf) as (
  ...
  select
    a.cpf,
    max(c.cep_endereco) filter (where c.cep_endereco is not null) as cep_cadastro,
    count(l.cadastro_id) as chamadas,
    max(l.cep_log) filter (where l.cep_log is not null) as cep_log
  from alvo a
...
select * from diag
where cep_cadastro is not null or cep_log is not null or chamadas=0
order by cpf;